...
`analyses.n_pcs` is a fixed integer by default (10 in the quickstart
example), but every run also compares that choice against the sequential
...
number (requires the `LEA` package; the comparison figure itself degrades
gracefully -- a `WARNING`, not a hard failure -- if `LEA` is unavailable
and a fixed `n_pcs` was requested).

![PCA component significance (Tracy-Widom test) from the quickstart example: percent of total variance vs. component index, with the Tracy-Widom-significant components highlighted and the fixed 10-component choice marked separately](figures/06b_PCA_Tracy_Widom_test.png)
...
diagnostics that led to it. In the quickstart example, the consensus is
**K=2** -- mean cross-validation success, Calinski-Harabasz separation,
Davies-Bouldin index, and replicate-membership RMSE all independently agree
on it (`22e_DAPC_K_selection_votes.tsv`). K=2's own `replicate_max_rmse` is
effectively 0 (~2e-20, fully reproducible, with every replicate's cluster
membership correlating at 1.0); most other K values in the same run exceed
the threshold and are not shown here for that reason -- K=3 is a real
exception (`replicate_max_rmse` ~1.5e-17, also fully reproducible) and is
used below purely to illustrate figures that a two-cluster model, by
...
`21_DAPC_diagnostics.tsv` for that K (30 for the consensus K=2). This figure
shows the full diagnostic behind that choice, not just the summary: every
individual bootstrap replicate's outcome (the semi-transparent points -- the
same real per-replicate variability `adegenet::xvalDapc()` itself computes
but normally discards after averaging), the mean curve connecting them, the
full random-chance reference band (2.5/50/97.5%, not just its median), and
the selected PC count marked directly on the curve. A tight cluster of
replicate points at each candidate count is reassuring; a wide spread means
the "best" count is closer to a coin flip among several similarly-plausible
values than a clean, confident peak -- K=2's own curve here is the tight
case, assignment success at or near 100% from a fairly small PC count
onward.

![DAPC PC-count cross-validation at the consensus K=2 from the quickstart example: every individual bootstrap replicate's outcome, the mean success curve, the full random-chance reference band, and the selected PC count](figures/12b_DAPC_xval_K2.png)

`22b_DAPC_membership_K<k>.tsv` and this figure report each sample's
posterior membership probability for its assigned cluster. At the consensus
K=2, membership is essentially unanimous (mean assignment success 1.0) --
the two-cluster split this dataset supports most strongly by every
criterion above.

![DAPC membership probabilities at the consensus K=2 from the quickstart example](figures/14_DAPC_membership_K2.png)

A K=2 model has only one discriminant axis by construction (`n.da`, capped
at K-1 by `adegenet::dapc()`) -- there is no second axis to plot against the
first, so DAPC's own 2D scatter plot (with its eigenvalues inset, described
next) is not produced for K=2 at all; `dapc$eig` for K=2 is a single value
(100% of the between-group variance, trivially, since it is the only axis).
K=3 -- also fully reproducible here, just not the automatic consensus -- is
shown below instead, purely to illustrate what that scatter and its
eigenvalues inset look like once a model has two or more discriminant axes.

A second, independent check on how many axes are worth trusting, separate
from the PC-retention question above, comes from the discriminant analysis
itself: `dapc$eig`, the between-group variance each retained discriminant
axis explains, drawn as a small inset directly on the scatter plot below (no
axis titles, positioned wherever the fewest sample points are, so it never
covers real data). A steep drop after the first axis or two means later
axes carry comparatively little real separating power; a flat,
undifferentiated set of bars across many axes is a sign the discriminant
step is not cleanly separating the data either. At K=3, LD1 explains 83.6%
of the between-group variance and LD2 the remaining 16.4% -- a steep,
reassuring drop given there are only `K - 1 = 2` axes to retain at K=3.
...
![DAPC scatter at K=3 from the quickstart example, illustrative only -- the consensus itself is K=2, which has no second discriminant axis to plot; the small inset in the emptiest quadrant shows each axis's own eigenvalue contribution](figures/11_DAPC_K3.png)
