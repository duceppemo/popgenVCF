test_that("identical descriptors are compatible and deterministic", {
  ...
  first <- compare_phase10_api_descriptors(baseline, candidate)
  second <- compare_phase10_api_descriptors(baseline, candidate)
  ...
  expect_true(validate_phase10_api_compatibility(first))
})
...
test_that("new operations and compatible schema advances are additive", {
  ...
  result <- compare_phase10_api_descriptors(baseline, candidate)
...
test_that("removed operations and major schema changes fail closed", {
  ...
  removed_result <- compare_phase10_api_descriptors(baseline, removed)
  expect_identical(removed_result$classification, "breaking")
  expect_false(removed_result$release_compatible)
  expect_error(
    validate_phase10_api_compatibility(removed_result),
    "requires explicit approval"
  )
  expect_true(validate_phase10_api_compatibility(removed_result, allow_breaking = TRUE))
  ...
  schema_result <- compare_phase10_api_descriptors(baseline, schema)
...
test_that("compatibility evidence detects tampering and renders a report", {
  result <- compare_phase10_api_descriptors(
    phase10_api_descriptor("1.0.0"),
    phase10_api_descriptor("1.0.0")
  )
  report <- phase10_api_compatibility_report(result)
  ...
  expect_error(
    validate_phase10_api_compatibility(result, allow_breaking = TRUE),
