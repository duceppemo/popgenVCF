...
normalize_relative_path <- function(path, root) {
  ...
  prefix <- paste0(root, "/")
  if (!startsWith(path, prefix)) stop("Asset is outside the release root: ", path, call. = FALSE)
  substring(path, nchar(prefix) + 1L)
}

collect_release_assets <- function(asset_dir, excluded = c("release-manifest.json", "release-SHA256SUMS.txt")) {
  require_release_packages()
  if (!dir.exists(asset_dir)) stop("Asset directory does not exist: ", asset_dir, call. = FALSE)
  ...
  if (!length(files)) stop("No release payload assets were found", call. = FALSE)
...
build_release_manifest <- function(asset_dir, package_name, package_version, release_id, git_tag,
                                   git_commit, r_version, workflow_name, workflow_run_id,
...
write_release_manifest <- function(manifest, asset_dir) {
  ...
  jsonlite::write_json(manifest, manifest_path, auto_unbox = TRUE, pretty = TRUE,
                       null = "null", digits = NA)
  checksum_lines <- vapply(manifest$payload_assets, function(asset) {
    sprintf("%s  %s", asset$sha256, asset$path)
  }, character(1))
  checksum_lines <- c(checksum_lines, sprintf(
    "%s  %s",
...
verify_release_manifest <- function(asset_dir) {
  ...
  if (anyDuplicated(expected_paths)) stop("Manifest contains duplicate asset paths", call. = FALSE)
  for (asset in assets) {
...
run_tamper_test <- function(asset_dir) {
  manifest <- jsonlite::read_json(file.path(asset_dir, "release-manifest.json"), simplifyVector = FALSE)
  ...
  if (!detected) stop("Tamper test failed: modified payload was accepted", call. = FALSE)
  invisible(TRUE)
}
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  manifest <- build_release_manifest(
    asset_dir = args[[1L]], package_name = args[[2L]], package_version = args[[3L]],
    release_id = args[[4L]], git_tag = args[[5L]], git_commit = args[[6L]],
    r_version = args[[7L]], workflow_name = args[[8L]], workflow_run_id = args[[9L]],
