...
validate_config <- function(cfg) {
  ...
  if (!identical(as.character(cfg$schema_version), "1.0")) {
    stopf("Unsupported configuration schema_version: %s", as.character(cfg$schema_version))
  }
  req <- c(cfg$input$vcf, cfg$input$metadata, cfg$output$directory)
  if (any(vapply(req, is.null, logical(1)))) stop("VCF, metadata, and output directory are required", call. = FALSE)
  if (!file.exists(cfg$input$vcf)) stopf("VCF not found: %s", cfg$input$vcf)
  if (!file.exists(cfg$input$metadata)) stopf("Metadata not found: %s", cfg$input$metadata)
  ...
  fixed_changed <- !isTRUE(all.equal(cfg$qc$ld_r2, 0.2)) ||
    !isTRUE(all.equal(cfg$qc$max_variant_missing, 0.2))
  if (fixed_changed) {
    warning(
      "The fixed QC contract requires max_variant_missing = 0.2 and LD r^2 = 0.2; overriding configured values.",
      call. = FALSE
    )
  }
  cfg$qc$ld_r2 <- 0.2
  cfg$qc$max_variant_missing <- 0.2
  cfg$qc$ld_slide_max_bp <- Inf
  cfg$qc$ld_slide_max_n <- 50L
  cfg$qc$ld_start_pos <- "first"
  ...
  if (!is.finite(cfg$analyses$chromosome_min_snps) || cfg$analyses$chromosome_min_snps < 2L) {
    stop("analyses.chromosome_min_snps must be >= 2", call. = FALSE)
  }
  if (!is.finite(cfg$analyses$bootstrap$replicates) || cfg$analyses$bootstrap$replicates < 0L) {
    stop("bootstrap.replicates must be >= 0", call. = FALSE)
  }
  ...
  if (!cfg$input$metadata_header %in% c("auto", "yes", "no", "true", "false")) {
    stop("input.metadata_header must be auto, yes, or no", call. = FALSE)
  }

  if (!is.finite(cfg$analyses$structure$replicates) || cfg$analyses$structure$replicates < 1L) {
    stop("analyses.structure.replicates must be >= 1", call. = FALSE)
  }
  if (!is.finite(cfg$analyses$structure$reproducibility_rmse) || cfg$analyses$structure$reproducibility_rmse < 0) {
    stop("analyses.structure.reproducibility_rmse must be non-negative", call. = FALSE)
  }
  if (!is.finite(cfg$analyses$structure$minimum_cluster_correlation) ||
      cfg$analyses$structure$minimum_cluster_correlation < -1 ||
      cfg$analyses$structure$minimum_cluster_correlation > 1) {
    stop("analyses.structure.minimum_cluster_correlation must be between -1 and 1", call. = FALSE)
  }

  if (isTRUE(cfg$analyses$admixture$enabled)) {
    ac <- cfg$analyses$admixture
    if (is.null(ac$plink_prefix) || !nzchar(ac$plink_prefix)) {
      stop("analyses.admixture.plink_prefix is required when ADMIXTURE is enabled", call. = FALSE)
    }
    if (is.null(ac$q_sample_file) || !file.exists(ac$q_sample_file)) {
      stop("A valid analyses.admixture.q_sample_file is required", call. = FALSE)
    }
  }
  if (isTRUE(cfg$analyses$faststructure$enabled)) {
    fc <- cfg$analyses$faststructure
    if (is.null(fc$plink_prefix) || !nzchar(fc$plink_prefix)) {
      stop("analyses.faststructure.plink_prefix is required", call. = FALSE)
    }
    if (is.null(fc$q_sample_file) || !file.exists(fc$q_sample_file)) {
      stop("A valid analyses.faststructure.q_sample_file is required", call. = FALSE)
    }
  }
  if (isTRUE(cfg$analyses$snmf$enabled)) {
    sc <- cfg$analyses$snmf
    if (is.null(sc$geno_file) || !file.exists(sc$geno_file)) {
      stop("A valid analyses.snmf.geno_file is required", call. = FALSE)
    }
    if (is.null(sc$q_sample_file) || !file.exists(sc$q_sample_file)) {
      stop("A valid analyses.snmf.q_sample_file is required", call. = FALSE)
    }
  }
  cfg
}
