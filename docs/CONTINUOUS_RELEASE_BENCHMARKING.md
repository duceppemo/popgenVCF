...
**As of 2026-09-14, the `synthetic` tier's `continuous_benchmarks` comparison no longer gates a release** (see the dated section below): its evidence is still recorded and reported, but `scripts/build_release_benchmark_archive.R` excludes it from the `stop()`-triggering decision. `canonical`/`medium`/`large` keep gating normally -- their seconds-scale absolute runtimes make CI-runner noise proportionally negligible, unlike `synthetic`'s sub-second scale.
...
## The synthetic tier's own loosened budget still wasn't enough (2026-09-14)

The `v1.0.13` release-cut's "Scientific release archive" workflow failed on `continuous release benchmark comparison failed` -- three times in a row (the original tag-push run, a manual rerun, and a `workflow_dispatch` diagnostic run), all showing the same order-of-magnitude runtime inflation on the `synthetic` tier: `runtime_seconds` observed `0.917` vs baseline `0.128` (~7.2x), well past even the already-widened `1.5x` budget from the 2026-08-21/22 fix above.

Root-caused properly rather than just retrying until green:

1. **Ruled out a real code regression.** `v1.0.13` changed zero R code versus `v1.0.12` (only vignette PNG downsampling, the `packaging/bioconda/` draft recipe, and wiki/version-metadata files). Confirmed directly: checked out both tags into isolated `git worktree`s and ran the identical `benchmark_tier_spec("synthetic", ...)` call against each under the same local R installation. `v1.0.12`: `runtime_median = 0.151s`. `v1.0.13` (`main`): `runtime_median = 0.142s`. Effectively identical, and both close to the CI baseline's `0.128s` -- nothing to suggest a real slowdown.
2. **Ruled out a same-process measurement-contamination bug.** `build_release_benchmark_archive.R` runs `run_scientific_validation(integration = TRUE, ...)` and `run_population_structure_validation()` immediately before the benchmark tier loop, in the same R process, raising the question of whether that prior work's memory/GC state was polluting the timing. Reproduced CI's exact sequence locally (validation calls immediately followed by the benchmark, same process, no isolation) -- still `runtime_median = 0.143s`. Not the cause.
3. **Conclusion: genuine GitHub Actions shared-runner noise**, consistent with the exact failure mode already documented above (2026-08-21/22): a sub-second absolute runtime leaves almost no real margin, and this occurrence was simply a larger noise spike than the one that originally motivated the `1.5x` synthetic-tier budget.

Also surfaced along the way: `release-benchmark-archive.yml`'s "Package archive"/"Upload workflow artifacts" steps only ran on success, so every one of these three failures discarded the very evidence (`release_comparison.tsv`, `continuous_benchmarks.tsv`) needed to diagnose it -- forcing local reproduction from scratch instead of just reading the artifact. Fixed with `if: always()` on both steps (commit `346107f`), which is what actually made steps 1-3 above possible without re-deriving everything by hand.

**Policy change**: the `synthetic` tier's `continuous_benchmarks` comparison no longer gates a release (`scripts/build_release_benchmark_archive.R`, `gating_continuous_status` computed excluding the `synthetic` tier from the `stop()` decision). This mirrors a policy this codebase already applied elsewhere for the identical measurement: `new_performance_benchmark_spec()`'s own `gating = FALSE` already made the separate "golden"/performance comparison advisory-only, and `v1.0.13`'s own `release_comparison.tsv` shows exactly that inconsistency in the same report -- the `performance` row flagged `regressed: TRUE` but `gating: FALSE` for this identical runtime measurement, while the `continuous_benchmarks` mechanism measuring the same thing was hard-failing the whole workflow. The evidence is still fully recorded (`continuous_benchmarks.tsv`/`.json`/`.md`, trend history) for visibility; it simply no longer blocks a release on its own. `canonical`/`medium`/`large` are unaffected and keep gating normally -- their multi-second absolute runtimes make the same class of CI noise proportionally negligible, exactly the reasoning `release_performance_budget_for_tier()`'s own comment already gives for treating `synthetic` differently.
