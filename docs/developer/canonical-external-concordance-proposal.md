...
**2026-08-22 -- Marc-Olivier Duceppe:** reviewed and approved all 5 manual checklist items against this commit-`50fb45a` evidence. `tool_inventory`, `command_and_version`, `numerical_comparisons`, and `diagnostic_interpretation` approved as verified. `estimator_compatibility` approved as-is despite the `amova`/`poppr` self-comparison nuance (that record compares popgenVCF's own value against itself, not an independent cross-tool check) -- accepted since the underlying evidence is technically correct; relabeling its role/wording is deferred to a future documentation pass, not blocking. `external_concordance-gate-record.json` materialized (`status: passed`, checksum-bound to the retained evidence, `approval.state: approved`) via `scripts/write_scientific_review_gate_record.R`, ready to fold into a future release-candidate evidence index rebuild.
